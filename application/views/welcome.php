...
<body>
    ...
    <?php
        ...
        if ($count % $this->config->item('supervisor_cols') === 1) {
        ...
        <table class="table table-bordered table-condensed table-striped">
            <tr><th colspan="4">
                <a href="<?php echo $ui_url; ?>"><?php echo $name . ' (' . count($procs) . 'x)'; ?></a> <?php if($this->config->item('show_host')){ ?><i><?php echo $parsed_url['host']; ?></i><?php } ?>
                <?php
    ...
    <?php
        if ($count % $this->config->item('supervisor_cols') === 0 || $count === count($list)) {
            echo '</div></div>';
        }
    }
